...
public class Bai4 {
    ...
    public static void main(String[] args) {
        ...
        // b. Khởi tạo ngẫu nhiên điểm của học sinh từ 1 đến 10
        for (int i = 0; i < n; i++) {
            diemHocSinh[i] = random.nextInt(10) + 1;
        }

        // c. Xuất danh sách học sinh cùng với năng lực học tập
        ...
        // d. Sắp xếp danh sách học sinh theo thứ tự giảm dần theo lớp
        System.out.println("\nDanh sách học sinh sau khi sắp xếp theo lớp:");
        sortHocSinhTheoLop(tenHocSinh, lopHocSinh, diemHocSinh);
        for (int i = 0; i < n; i++) {
            System.out.println("Tên: " + tenHocSinh[i] + ", Lớp: " + lopHocSinh[i] + ", Điểm: " + diemHocSinh[i]);
        }

        // e. In ra thông tin học sinh có điểm cao nhất
        ...
        // f. Tính điểm trung bình của học sinh
    ...
    // Hàm sắp xếp danh sách học sinh theo thứ tự giảm dần theo lớp
    public static void sortHocSinhTheoLop(String[] tenHocSinh, String[] lopHocSinh, double[] diemHocSinh) {
        // Sử dụng mảng chỉ mục để sắp xếp đồng thời cả tên, lớp và điểm
    ...
    // Hàm tính điểm trung bình của các học sinh
